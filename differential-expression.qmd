...
# Combine inactive categories into one "other" category
exogenous_counts_all <- exogenous_rna_count_data
exogenous_rna_count_data <- exogenous_rna_count_data %>%
  ...
  group_by(sample_unit, sequence_name, category) %>%
  summarize(count = sum(count), .groups = "keep")

# Reformat exogenous counts
...
# Color palette
human_gene_colors <- c("#a9a9a9", "#dddddd")
exogenous_gene_colors <- c("#d95f02", "#7570b3", "#1b9e77")
na_color <- "#777777"
...
# Remove labels
p_nolabels <- p
p_nolabels$layers <- p$layers[-2]
ggsave("diffexp-batch2-day1-nolabels.png", dpi = 300)
```
...
p <- ggplot(
  ...
  ggrepel::geom_label_repel(
    ...
    aes(label = rna_species, segment.colour = "black", color = significant),
    min.segment.length = 0,
    max.overlaps = 30,
    size = 3,
    force_pull = 0.5,
...
p_nolabels <- p
p_nolabels$layers <- p$layers[-2]
ggsave("diffexp-batch2-day2-nolabels.png", dpi = 300)
```
...
p <- ggplot(
  ...
  ggrepel::geom_label_repel(
    ...
    aes(label = rna_species, segment.colour = "black", colour = significant),
    min.segment.length = 0,
    max.overlaps = Inf,
    size = 3,
    force_pull = 0.5,
...
p_nolabels <- p
p_nolabels$layers <- p$layers[-2]
ggsave("diffexp-batch3-day1-nolabels.png", dpi = 300)
```
...
p_nolabels <- p
p_nolabels$layers <- p$layers[-2]
ggsave("diffexp-batch3-day2-nolabels.png", dpi = 300)
```
