...
pegrna_plots <- function(rna_species, normalization_factor, ylim, ylab, vlines = NA) {
  par(mfrow = c(2, 1))
  ...
  for (s in (sample_units %>%
    ...
    for (vline in vlines) {
      ...
      text(x = vline, y = 0, as.character(vline), pos = 2, cex = 0.75, lwd = 0.5)
    }
    last_day <- day
  }
}
...
pegrna_plots(
  ...
  vlines = c(331, 357, 363, 388, 425, 451, 456)
)
```
#### PJY300 (epegRNA)
...
## Start and End Position Histograms
...
start_counts_pjy103 <- bedpe_data %>% 
  dplyr::filter(chrom1 == "PJY103") %>%
  dplyr::group_by(sample, chrom1, chrom1Start) %>%
  dplyr::tally(name = "count") %>%
  mutate(counts_per_pegrna_reads = count / exogenous_rna_mapped_reads) %>%
  left_join(sample_units, by = c("sample" = "sample_unit"))

start_counts_pjy103_summary <- 
  plyr::ddply(start_counts_pjy103,
              c("sample_name", "chrom1Start", "cell_line", "day"),
              summarise,
              N = length(counts_per_pegrna_reads),
              mean = mean(counts_per_pegrna_reads),
              sd = sd(counts_per_pegrna_reads),
              se = sd / sqrt(N))

ggplot(start_counts_pjy103_summary,
       aes(x = chrom1Start, y = mean, color = cell_line, group = 1)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 2) +
  ggtitle("PJY103 START normalized to pegRNA") +
  xlim(250, 510) +
  geom_point() +
  facet_grid(day ~ .)
```
...
```{r fig.width=12, fig.height=5}
end_counts_pjy103 <- bedpe_data %>% 
  dplyr::filter(chrom1 == "PJY103") %>%
  dplyr::group_by(sample, chrom1, chrom2End) %>%
  dplyr::tally(name = "count") %>%
  mutate(counts_per_pegrna_reads = count / exogenous_rna_mapped_reads) %>%
  left_join(sample_units, by = c("sample" = "sample_unit"))

end_counts_pjy103_summary <- 
  plyr::ddply(end_counts_pjy103,
              c("sample_name", "chrom2End", "cell_line", "day"),
              summarise,
              N = length(counts_per_pegrna_reads),
              mean = mean(counts_per_pegrna_reads),
              sd = sd(counts_per_pegrna_reads),
...
ggplot(end_counts_pjy103_summary,
       aes(x = chrom2End, y = mean, color = cell_line, group = 1)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 2) +
  ggtitle("PJY103 END normalized to pegRNA") +
  xlim(250, 510) +
  geom_point() +
  facet_grid(day ~ .)
```
...
```{r fig.width=12, fig.height=5}
start_counts_pjy300 <- bedpe_data %>% 
  dplyr::filter(chrom1 == "PJY300") %>%
  dplyr::group_by(sample, chrom1, chrom1Start) %>%
  dplyr::tally(name = "count") %>%
  mutate(counts_per_pegrna_reads = count / exogenous_rna_mapped_reads) %>%
  left_join(sample_units, by = c("sample" = "sample_unit"))

start_counts_pjy300_summary <- 
  plyr::ddply(start_counts_pjy300,
              c("sample_name", "chrom1Start", "cell_line", "day"),
              summarise,
              N = length(counts_per_pegrna_reads),
              mean = mean(counts_per_pegrna_reads),
...
ggplot(start_counts_pjy300_summary,
       aes(x = chrom1Start, y = mean, color = cell_line, group = 1)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 2) +
  ggtitle("PJY300 START normalized to pegRNA") +
  xlim(250, 510) +
  geom_point() +
  facet_grid(day ~ .)
```
...
end_counts_pjy300_summary <- 
  plyr::ddply(end_counts_pjy300,
              c("sample_name", "chrom2End", "cell_line", "day"),
              summarise,
              N = length(counts_per_pegrna_reads),
              mean = mean(counts_per_pegrna_reads),
...
ggplot(end_counts_pjy300_summary,
       aes(x = chrom2End, y = mean, color = cell_line, group = 1)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 2) +
  ggtitle("PJY300 END normalized to pegRNA") +
  xlim(250, 510) +
  geom_point() +
  facet_grid(day ~ .)
```
