...
cis_active_pbs_quantification <- tibble(
  sample_unit = character(),
...
for (rna_species in names(rna_species_plot_data)) {
  ...
  for (sample_unit in names(rna_species_data)) {
    ...
    cis_active_pbs_quantification <- cis_active_pbs_quantification %>%
      add_row(
        sample_unit = sample_unit,
        sequence_name = rna_species,
...
summarized_three_prime_quantification
write_tsv(summarized_three_prime_quantification, "summarized_three_prime_quantification.tsv")

```

```{r}
#| label: plot_threeprime-quantification
...
p <- ggplot(
  ...
  aes(
    ...
    x = interaction(sequence_name, day),
  )
) +
