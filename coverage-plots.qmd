...
for (rna_species in rna_mixes %>%
  ...
  for (day in days_to_plot) {
    ...
    normalization_labs <- c(
      "per million human small rna",
      "as fraction of reads in category"
    )
    names(normalization_labs) <- normalizations_to_plot
    ...
    # Line 1 bp after start of annotation
    annotation_lines <- rbind(
      annotation_lines,
      annotations %>%
        filter(name == "nick") %>%
        mutate(x = chromStart + 1)
    )

    # Rectangles
    annotation_rectangles <- annotations %>%
      filter(name == "edit") %>%
      mutate(xmin = chromStart, xmax = chromEnd)
    ...
    annotation_labels <- rbind(
      ...
      mutate(normalization = "human_small_rna") %>%
      mutate(
        label = case_when(
    ...
    p <- ggplot(
      ...
      scale_linetype_discrete(
        ...
        labels = c("Active in cis", "Active in trans")
      ) +
      guides(
      ...
      xlab("Position") +
      facet_wrap(
        facets = ~normalization, scales = "free_y", ncol = 1,
        strip.position = "left",
        labeller = labeller(normalization = normalization_labs)
      ) +
      ...
      geom_rect(
        data = annotation_rectangles,
        xmin = annotation_rectangles$xmin,
        xmax = annotation_rectangles$xmax,
        ymin = min(extendrange(ylims_human, f = 0.10)),
        ymax = max(extendrange(ylims_human, f = 0.10)),
        fill = "#8da0cb7f",
        color = "#8da0cbFF",
        lwd = 0.5,
        inherit.aes = FALSE
